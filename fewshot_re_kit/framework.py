...
class FewShotREFramework:
    ...
    def train(self,
              model,
              model_name,
              B, N_for_train, N_for_eval, K, Q,opt,
              na_rate=0,
              learning_rate=1e-1,
              lr_step_size=20000,
              weight_decay=1e-5,
        ...
        if bert_optim:
            ...
            # about poly
            if not opt.polyformer_mode:  # not use
                parameters_to_optimize = list(model.named_parameters())
            else:  # use poly
                translayers = model.polyformer.polyformer_layers
                if opt.polyformer_mode == 'source':
                    poly_opt_mode = opt.poly_source_opt  # allpoly
                else:
                    poly_opt_mode = opt.poly_target_opt  # k
                ...
                if poly_opt_mode == 'allnet':
                    ...
                    for poly_opt_mode in poly_opt_modes:
                        if poly_opt_mode == 'allpoly':
                            for i in list(translayers.named_parameters()):
                                #optimized_params += tuple(i)
                                optimized_params.append(tuple(i))
                            # optimized_params += [tuple(translayers.named_parameters())]
                        elif poly_opt_mode == 'inator':
                            #optimized_params += [translayer.in_trans.named_parameters() for translayer in translayers]
                            for translayer in translayers:
                                for i in list(translayer.in_trans.named_parameters()):
                                    optimized_params.append(tuple(i))
                        elif poly_opt_mode == 'k':
                                #optimized_params += [translayer.in_trans.key.named_parameters() for translayer in translayers]
                            #for i in list(translayer.in_trans.key.named_parameters() for translayer in translayers):
                             #   optimized_params.append(tuple(i))
                            for translayer in translayers:
                                for i in list(translayer.in_trans.key.named_parameters()):
                                    optimized_params.append(tuple(i))
                        elif poly_opt_mode == 'v':
                            #optimized_params += [translayer.in_trans.value.named_parameters() for              translayer in translayers]
                            for translayer in translayers:
                                for i in list(translayer.in_trans.val.named_parameters()):
                                    optimized_params.append(tuple(i))
                        elif poly_opt_mode == 'q':
                            #optimized_params += [translayer.in_trans.query.named_parameters() for translayer in      translayers]
                            for translayer in translayers:
                                for i in list(translayer.in_trans.query.named_parameters()):
                                    optimized_params.append(tuple(i))
                            '''
                        # optimize the segmentation head as well.
                        elif poly_opt_mode == 'h':
                            # Only for VanillaUNet
                            assert opt.model == 'proto'
                            optimized_params += [model.outc.named_parameters()]
                            '''
                    parameters_to_optimize = optimized_params

            #affine
            if opt.bn_opt_scheme == 'affine':
                bn_params = []
                for layer in model.modules():
                    if isinstance(layer, nn.LayerNorm):
                        bn_params.extend(list(layer.named_parameters()))
                parameters_to_optimize += bn_params

            no_decay = ['bias', 'LayerNorm.bias', 'LayerNorm.weight'] #代表不优化
            parameters_to_optimize = [
                {'params': [p for n, p in parameters_to_optimize      #so n-name p-param?
                    if not any(nd in n for nd in no_decay)], 'weight_decay': 0.01},
                {'params': [p for n, p in parameters_to_optimize
                    if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}
            ]

            # parameters_to_optimize = list(itertools.chain.from_iterable(optimized_params))
        ...
        for it in range(start_iter, start_iter + train_iter):
            if pair:
                ...
                if torch.cuda.is_available():
                    ...
                    label = label.cuda()
                #test = torch.cuda.current_device() # which gpu
            ...
            if (it + 1) % val_step == 0:
                ...
                if acc > best_acc:
                    ...
                    torch.save({'state_dict': model.state_dict()}, save_ckpt, _use_new_zipfile_serialization=False)
                    best_acc = acc
                iter_loss = 0.
                iter_loss_dis = 0.
                iter_right = 0.
