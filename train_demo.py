...
import models
# from models.polyformer import PolyformerLayer, Polyformer  # increase
...
def main():
    ...
    parser.add_argument('--train', default='val_pubmed',
                        help='train file')
    parser.add_argument('--val', default='val_semeval',
                        help='val file')
    parser.add_argument('--test', default='val_semeval',
                        help='test file')
    ...
    parser.add_argument('--batch_size', default=1, type=int,  # 这里改成2就可以跑了
                        help='batch size')
    parser.add_argument('--train_iter', default=6000, type=int, #30000
    ...
    parser.add_argument('--val_step', default=1000, type=int, #2000
    ...
    parser.add_argument('--hidden_size', default=230, type=int,
                        help='hidden size')
    parser.add_argument('--load_ckpt', default='checkpoint/s-usep-proto-bert-train_wiki-val_wiki-5-5.pth.tar'#None,
                        ,help='load ckpt')
    ...
    # Polyformer
    parser.add_argument("--polyformer", dest='polyformer_mode', type=str, default='target',
                        choices=[None, 'none', 'source', 'target'],
                        help='Do polyformer training.')
    ...
    parser.add_argument("--num_head", dest='num_head', type=int, default='2',
                       help='the number of head')
    #few shot
    parser.add_argument("--bnopt", dest='bn_opt_scheme', type=str, default='affine',
    ...
    if opt.polyformer_mode == 'none':
        opt.polyformer_mode = None

    if opt.polyformer_mode:
        ifuse = 'usep'
        print("Do Polyformer training")
        if opt.polyformer_mode == 'source':
            opt.tie_qk_scheme = 'shared'  # in source, share qk
        else:
            opt.tie_qk_scheme = 'loose'  # in target, loose q and k.to update k?
    else:
        ifuse = 'unusep'
        opt.tie_qk_scheme = 'shared'  # but tie_qk_scheme dont't define?
    ...
    #a = [model_name, encoder_name, opt.train, opt.val, '5', '5']
    if opt.val == 'val_wiki':
        region = 's'
    else:
        region = 't'
    a = [region, ifuse, model_name, encoder_name, opt.train, opt.val, str(N), str(K)]
    prefix = '-'.join(a)
    ...
    if model_name == 'proto':
        model = Proto(sentence_encoder, dot=opt.dot, polyformer_args=opt)
    elif model_name == 'gnn':
    ...
    if not opt.only_test:
        ...
        framework.train(model, prefix, batch_size, trainN, N, K, Q, opt,
